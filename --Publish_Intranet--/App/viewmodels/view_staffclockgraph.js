define(['services/logger', 'services/datacontext', 'services/helpers'],
    function (logger, dataContext, helpers) {
            ...
            for (var i = 0; i < length; i++) {
                if (i > 0) {
                    switch (Math.floor(i % 2)) {
                        case 0: colour = 'blue'; break; // In the office
                        case 1: colour = 'green'; break; // Before work
                    }
                }
        ...
        function assembleGraphs(graphList) {
            var options = { xaxis: { mode: "categories", tickLength: 0 }, yaxis: { tickSize: 1 }, series: { bars: { show: true, barWidth: .9, align: "center" }, stack: true } };
            for (var i = 0; i < graphList.length; i++) {
                if (graphList[i].length >= 1) {
                    //options.xaxis.ticks = xAxisDateName[0];
                    var lookfor = "#graph" + i;
