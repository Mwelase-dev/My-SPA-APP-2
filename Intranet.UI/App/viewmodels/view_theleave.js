define(['services/datacontext', 'services/helpers', 'durandal/app', 'services/helpers'],
    function (datacontext, helpers, app, helper) {
        ...
        var enableChangeStatus = ko.observable(false);

        function activate(ctx) {
            //initialLeaveDays = theleave().staffMember().daysDue();
           // initSickDays = theleave().staffMember().sickDaysAvilable();
            /*return datacontext.getPublicHolidays(disabledDays).then(function () {
                return datacontext.getCurrentUser(manager).then(function () {
                    man1Id = theleave().staffMember().staffManager1Id();
                    man2Id = theleave().staffMember().staffManager2Id();
                    manId = manager().staffId();
                    canApprove((man1Id == manId || man2Id == manId) && (theleave().leaveStatus() == datacontext.leaveStatusEnum.pending) && isEditing() == false);
                }).then(function () {
        ...
        function refresh(leaveId) {

            return Q.fcall(function () {
                return datacontext.getLeave(theleave,leaveId).then(function() {
                    ...
                    enableChangeStatus(theleave().leaveStatus() == 2);
                });
               // return theleave(datacontext.getLeave(leaveId));
        ...
        function viewAttached() {
            ...
            var test = theleave();
            //leaveDays(theleave().displayLeaveDays);
            //displayLeaveHours(theleave().displayLeaveHours);
            //theleave().displayLeaveStart = + " " + theleave().displayLeaveHours;
            //theleave().displayLeaveEnd = + " " + theleave().displayLeaveHours;
            //leaveDays(helpers.calculateLeaveDays(theleave().leaveDateStart(), theleave().leaveDateEnd()));
            //if (!isEditing()) return;
        ...
        function configureStartDatePicker() {
            $("#leaveStart").datetimepicker(
                ...
                }).datepicker("setDate", theleave().leaveDateStart());
        ...
        function configureEndDatePicker() {
            $("#leaveEnd").datetimepicker(
                ...
                }).datepicker("setDate", theleave().leaveDateEnd());
        ...
        var vm = function (leave, edit) {
           ...
           // theleave(leave);
            ...
            this.close = close;
            this.theleave = theleave;
            this.enableChangeStatus = enableChangeStatus;
        };
